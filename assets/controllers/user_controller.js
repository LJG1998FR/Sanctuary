...
export default class extends Controller {
    ...
    connect() {
        ...
        document.querySelectorAll('.delete-checkbox').forEach(user => {
            user.addEventListener('click', () => {this.onSelected(user)});
        });
    ...
    onSelected(user){
        var userId = parseInt(user.dataset.userid);
        var userIndex = _.indexOf(this.idsToDelete, userId);

        if (userIndex === -1) {
            this.idsToDelete.push(userId);
        } else {
            this.idsToDelete.splice(userIndex, 1);
        }
    ...
    async onDeleteSelected(){
        ...
        if(confirm("Are you sure you want to delete these users?") == true){
            try {
                const response = await fetch("http://127.0.0.1:8000/admin/users/deleteSelected", {
                    method: "POST",
                    headers: {
    ...
    async onSearch(event){
        ...
        if(inputValue.length > 0){
           try {
                ...
                var turboFrame = document.getElementById('user_list');
