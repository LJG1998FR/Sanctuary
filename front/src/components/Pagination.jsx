    ...
    return (<nav aria-label="List navigation">
        <ul className="pagination justify-content-center">
            <li className={(page == 1) ? 'page-item disabled' : 'page-item'}>
                <Link className="page-link" to={{pathname: '/videos', search: buildSearch(page - 1)}} tabIndex="-1" aria-disabled={(page == 1) ? 'true' : 'false'}>{t('common.pagination.previous')}</Link>
            </li>
            ...
            <li className={(page >= nb_pages) ? 'page-item disabled' : 'page-item'}>
                <Link className="page-link" to={{pathname: '/videos', search: buildSearch(page + 1)}} aria-disabled={(page + 1 > nb_pages) ? 'true' : 'false'}>{t('common.pagination.next')}</Link>
            </li>
        </ul>
    </nav>);
}